...
## FarmerPlant: Moves to an empty soil tile and plants a seed
...
func enter() -> void:
	super()
	_is_planting = false
	if not entity.seed_queue.is_empty():
		entity._is_holding_product = false
		entity.held_item = entity.seed_queue[0]
...
func physics_update(_delta: float) -> void:
	...
	if not is_beside:
		...
		entity.helper_visual.play_dig()

func on_animation_finished(anim_state) -> void:
	if anim_state == HelperVisual.AnimState.DIG:
		entity._execute_plant()
		entity.held_item = null
		transition.emit("idle")
